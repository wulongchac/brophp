	...
	class Dpdo extends DB{
		...
		function query($sql, $method,$data=array()){
		    ...
		    $result=$stmt->execute($value);   //执行一个准备好的语句
				...
				if($cobj&& !$addcache){
					if($stmt->rowCount()>0){
						$cobj->delCache($this->tabName);	 //清除缓存
						Debug::addmsg("清除表<b>{$this->tabName}</b>在Memcache中所有缓存!"); //debug
					}
				}
				 ...
				 switch($method){
					...
					case "total":  //返回总记录数
						$row=$stmt->fetch(PDO::FETCH_NUM);
						 ...
						 if($addcache){
						 	$cobj->addCache($this->tabName, $memkey, $row[0]);
						 }
		 ...
		 * @param $sql
		 * @param array $data
		 * $data=array(1,"tabName",@rs)
		 **/
		function proc($procName,$data=array(),$is_use_out=true){
			try{
				$pdo=self::connect();
				$param_data =array();
				$sqlparam = "";
				...
				foreach($data as $key=>$value){
					if($sqlparam!="")$sqlparam.=",";
					if($sqlcol!="")$sqlcol.=",";
					if(is_bool(strpos($value,"@"))){
						$sqlparam.="?";
						array_push($param_data,$value);
					}else{
						$sqlparam.=$value;
						$sqlcol.=$value;
					}
				...
				$stmt=$pdo->prepare("call $procName($sqlparam);");
				$result=$stmt->execute($param_data);
				if($result===false){
					return $result;
				}
				if($is_use_out){
					...
					return $pdo->query("select $sqlcol")->fetch(PDO::FETCH_ASSOC);
				}else{
